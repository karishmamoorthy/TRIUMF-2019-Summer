...
def get_S(alpha, v0=1, ht_like_4=1, kck=0):
    ...
    V_Minkowski= lambda x: ht_like_4*((2 - 3*alpha)*(x/v0)**2 - (x/v0)**4  + alpha*(x/v0)**6)  #Must be a dimensionless potential.

    V= lambda x: -1*V_Minkowski(x)
    
    mu_sq = (2 - 3*alpha)*ht_like_4/(v0**2)
    A = ht_like_4/(v0**4)
    lam = ht_like_4*alpha/(v0**6)

    phi_localmax = v0
    if A**2 - 3*mu_sq*lam <= 0:
        phi_localmin = np.sqrt(A/(3*lam))
    else:
        phi_localmin = np.sqrt((A + np.sqrt(A**2 - 3*mu_sq*lam))/(3*lam))
    
    if A**2 - 4*mu_sq*lam <= 0:
        phi_localzero = np.sqrt(A/(2*lam))
    else:
        phi_localzero = np.sqrt((A - np.sqrt(A**2 - 4*mu_sq*lam))/(2*lam))

    V_pr = lambda x: -1*(2*mu_sq*x - 4*A*x**3  + 6*lam*x**5)
    ...
    if case:   
        ...
        radius_function = sci.interp1d(k[0], k[2])
        
        k_short_1 = np.linspace(k[1][0], 0.9*k[1][0], 1000)
        k_short_0 = inverse_function(k_short_1)
        k_short_2 = radius_function(k_short_0)
        
        S_integrand = (0.5*(k_short_2)**2 - V(k_short_1))*k_short_0*k_short_0
        
        new_phi_vals = np.linspace(0, phi_localzero, int(phi_localzero/dr) + 1)
        
        #print("S_integrand[-1] = ", S_integrand[-1])
        #print()
    
        S = 4*np.pi*function_integrator(S_integrand, k_short_0) + 4*np.pi*(bubble_radius**2)*function_integrator(np.sqrt(2*V_Minkowski(new_phi_vals)), new_phi_vals)
