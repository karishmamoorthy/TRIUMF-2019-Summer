...
def theory(alpha_theory, Lambda_theory, v_theory, dof=3, T_start=None, resolution=1000):
    ...
    file2 = open("S_alpha_246_template_alpha_vals.txt", "r")
    for line2 in file2:
        for word2 in line2.split(): 
            a_data_template.append(float(word2))
    file2.close()

    file2 = open("S_alpha_246_template_dof_3_S_vals.txt", "r")
    for line2 in file2:
        for word2 in line2.split(): 
            s_data_dof_3.append(float(word2))
    file2.close()
    
    file2 = open("S_alpha_246_template_dof_4_S_vals.txt", "r")
    for line2 in file2:
        for word2 in line2.split(): 
            s_data_dof_4.append(float(word2))
    file2.close()
    
    file2 = open("S_alpha_246_template_dof_3_R_vals.txt", "r")
    for line2 in file2:
        for word2 in line2.split(): 
            r_data_dof_3.append(float(word2))
    file2.close()
    
    file2 = open("S_alpha_246_template_dof_4_R_vals.txt", "r")
    for line2 in file2:
        for word2 in line2.split(): 
    ...
    compare2 = list(abs(a_data_template - 0.520))
    find_index2 = compare2.index(min(compare2))
    if a_data_template[find_index2] < 0.520:
        find_index2 = find_index2 - 1
    
    compare3 = list(abs(a_data_template - 0.532))
    find_index4 = compare3.index(min(compare3))
    if a_data_template[find_index4] < 0.532:
    ...
    V_Minkowski_N = lambda x_sq: mu_sq_N*x_sq - A_theory*x_sq**2 + lam_theory*x_sq**3
    x_pit_sq_N = (A_theory + np.sqrt(A_theory**2 - 3*mu_sq_N*lam_theory))/(3*lam_theory)
    
    field_energy_density_at_N = -1*V_Minkowski_N(x_pit_sq_N)
